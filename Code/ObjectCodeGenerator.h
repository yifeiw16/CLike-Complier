...
class QuaternaryWithInfo {  //(get,_,_,a)(^,^)(^,^)(1,y) 这个形式  左操，右操，左值
public:
	Quaternary q;
	VarInfomation info1;
	VarInfomation info2;
...
private:
	...
	map<string, set<string> >Avalue;  //变量地址描述数组AVALUE A存在了哪些寄存器中
	map<string, set<string> >Rvalue;  //寄存器描述数组 R里面存了哪些变量
