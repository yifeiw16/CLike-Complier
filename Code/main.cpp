...
{
	cout << "****************************************" << endl;
	cout << "            类C语言编译器"               << endl << endl;
	cout << "             2051454 王逸菲" << endl;
	cout << "****************************************" << endl << endl;
	cout << "系统默认读入类C语言文法文件grammar.txt" << endl << endl;

	string gfile_name = "grammar.txt";
	string b;
	cout << "请输入要读入的文法文件名：（如果选择默认文件，请输入default）\n";
	cin >> b;
	if (b != "default")
	{
		gfile_name = b;
	...
	try
	{
		...
		cout << endl << "正在进行目标代码生成，请稍后..." << endl;
		Generator generator(lr1_grammar.semantic_analysis.quaternion_list);

		//划分基本块
		generator.divideBlocks(lr1_grammar.semantic_analysis.computeFuncTable());
		//对基本块进行分析
		generator.analyseBlock();

		//生成目标代码
		generator.generate();
		cout << "目标代码生成已结束，可在../work_dir文件夹中查看\n1. 目标汇编代码文件Target_Code.asm。" << endl;
	}
	catch (int a)
	{
