...
void IntermediateCode::divideBlocks(vector<pair<int, string> > funcEnter) {
	...
	cout << "* *****************************************************************************";
	for (const auto& entry : funcEnter) {
		
		std::cout << "Function start at line: " << entry.first << ", Name: " << entry.second << std::endl;
	}

	// 遍历 vector 并打印每个四元组指令的内容
	for (const auto& instr : code) {
		std::cout << "Operation: " << instr.op << ", "
			<< "Source1: " << instr.src1 << ", "
			<< "Source2: " << instr.src2 << ", "
			<< "Destination: " << instr.des << std::endl;
	}
	...
	//先划分函数块
	//再划分基本块 基本块带有两个参数，指明它下面一个基本块是什么，如果是跳转指令，将指明之后的在它时候的两个基本块分别是什么
	...
	for (vector<pair<int, string> >::iterator iter = funcEnter.begin(); iter != funcEnter.end(); iter++) {//对每一个函数块
		...
		for (int i = iter->first; i != endIndex; i++) {
			if (code[i].op[0] == 'j') {
				if (code[i].op == "j") {//若操作符是j  无条件转移
					block_enter.push(atoi(code[i].des.c_str()));   //将destination字段加入
				}
